...
sea_coords <- function(df){
  ...
  #ggplot(coords_land,aes(x=decimalLongitude,y=decimalLatitude,col=.sea))+geom_point()
  new_coordinates<-filter(coords_land,.sea==FALSE)
  
  filter(cleaned_df, !df_compared) %>%
    select(species,decimalLongitude,decimalLatitude)
}

cleaned_coordinates <- sea_coords(cleaned_df);

super_cleaned_list<-list()
for(i in 1:length(cleaned_list)){
  super_cleaned_list[[i]]<-sea_coords(data.frame(cleaned_list[[i]]))
}
...
map_coral <- function(list_clean){
  for (l in 1:length(list_clean)) {
    corpoints <- st_as_sf(data.frame(list_clean[[l]]), coords = c(x="decimalLongitude", y="decimalLatitude"))
     ...
     #as_mapper(c("world"))
     maps::map("world")
     plot(r,add=TRUE)  # adds the worldmap to the raster on the same plot , add= TRUE
     #dev.off()
   }
}

map_coral(super_cleaned_list);
...
coords_all_sp <- list();
coords_all_sp <- combine_all_coords(super_cleaned_list);
...
r_all <- rasterize(df_all_coords, r_all, field = "species", fun= function(x, ...) {length(unique(na.omit(x)))})
maps::map("world")
plot(r_all,add=TRUE)
...
r_susc_count <- rasterize(GBIF_susc, r_susc, field = "species", fun= function(x, ...) {length(na.omit(x))})
...
```{r}
r_all_new<-r_all>10
r_all_newer<-r_all*r_all_new

r_all_newer<-setValues(r_all_newer,ifelse(getValues(r_all)>10,
                             getValues(r_all),
                             NA))
...
sus.df<-data.frame(num.sp=getValues(r_susc),num.obs=getValues(r_susc_count))
ggplot(sus.df,aes(x=num.obs,y=num.sp))+geom_point()

r_rel_susc_high <- r_rel_susc > 0.2 #half of the corals of considered site are highly susceptible corals!
pts_high_susc <- data.frame(rasterToPoints(r_rel_susc,fun=function(x){x>0.2}))

world<-ne_countries(scale="medium",returnclass="sf")
ggplot(data=world)+geom_sf()+geom_point(data=pts_high_susc,aes(x=x,y=y,color=layer))+scale_colour_gradientn(colours = c("#FFCCCC", "#FF9999","#FF6666","#CC3333", "#990000", "#660000"))

```

```{r}
relativate <- function(raster_trait, raster_sp_count) {
  rel_trait <- integer();
  r_rel <- raster();
  for (i in 1:ncol(raster_trait)){
   for (j in 1:nrow(raster_trait)) {
     rel_trait <- round(raster_trait$layer/raster_sp_count$layer)
     r_rel[i,j] <- rel_trait
   }
  }
  return(r_rel)
